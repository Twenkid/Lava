...
#Lava examples, modified with more debugging and conversion to HwCfg: Todor Arnaudov - Twenkid 7.4.2025      from lava.magma.core.run_configs import Loihi2HwCfg #Loihi2SimCfg
# Torch 2.6 CUDA 12.6 - it doesn't utilize the GPU, 750 Ti
...
class PySpikeInputModel(PyLoihiProcessModel):
    ...
    def __init__(self, proc_params):
        ...
        self.curr_img_id = 0 #00; # 0  #23-3-2025
    ...
    def run_post_mgmt(self):
        ...
        print(f"run_post_mgmt: self.ground_truth_label = {self.ground_truth_label}\n self.ground_truth_label={self.ground_truth_label}") #OK but it is not received? 5.4.2025
...
class PyOutputProcessModel(PyLoihiProcessModel):
    ...
    def run_post_mgmt(self):
        ...
        returns True.
        """
        gt_label = self.label_in.recv(); print("gt_label=", gt_label); print(gt_label[0])
        pred_label = np.argmax(self.spikes_accum)
        print(len(gt_label), gt_label.shape, pred_label, self.spikes_accum)
        self.gt_labels[self.current_img_id] = gt_label
        self.pred_labels[self.current_img_id] = pred_label
        print(gt_label, gt_label.item())
        print(f"self.current_img_id={self.current_img_id}")
        self.gt_labels[self.current_img_id] = gt_label[0];# pred_label #gt_label.item() if hasattr(gt_label, 'item') else gt_label
#was commented prev line once
...
if __name__ == "__main__":
    # Your mnist_clf.run() and other execution code goes here
    ...
    from lava.magma.core.run_configs import Loihi2SimCfg #Loihi2SimCfg

    # Create a run configuration with GPU backend
    #run_cfg = Loihi2System.default()
    #run_cfg.select_tag("py")  # Use PyPy backend which can leverage GPU

    # Specify GPU device if you have multiple GPUs
    #run_cfg.custom_param("backend", "pt")  # Use PyTorch backend
    #run_cfg.custom_param("device", "cuda:0")  # Use first GPU
    #run_cfg.custom_param("select_sub_proc_model", True)
    #run_cfg.custom_param(select_tag, 'fixed_pt')

    #from lava.utils.system import Loihi2System

    # Create a PyTorch-based run configuration
    """
    run_cfg = Loihi2System.default()
    run_cfg.select_tag("py")  # Use PyPy backend
    run_cfg.custom_param("backend", "pt")  # Use PyTorch backend
    run_cfg.custom_param("device", "cuda:0")  # Use first GPU
    run_cfg.custom_param("select_sub_proc_model", True)
    """
    ...
    for img_id in range(num_images):
        ...
        mnist_clf.run(
            ...
            #select_tag="floating_pt")) -- recognizes noise
